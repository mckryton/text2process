...
on addProcessActionToLane(pProcessDoc, pLaneId, pPreviousItemId, pType, pId, pText)
	...
	--TODO: instead of pOffsetX tell id of the previous item and calculate offset on the fly
	set vOffsetX to my getOffsetX(pProcessDoc, pPreviousItemId)
	
	-- NOTE: the whole method is a workaround for not being able to move grouped shapes in OmniGraffle
